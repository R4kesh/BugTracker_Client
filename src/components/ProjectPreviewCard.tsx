...
// Define a TypeScript interface for the project structure
interface Project {
  id: string; // Assuming the ID is a string, adjust accordingly
  name: string;
  description: string;
  status: "completed" | "in-progress" | "on-hold" | "unknown"; // Adjust statuses as needed
  startDate: string; // Assuming the date is in ISO format or similar string
}

export function ProjectPreviewCard() {
  const [projects, setProjects] = useState<Project[]>([]); // Specify the type for projects state
  ...
  // Handle page change
  const handlePageChange = (pageNumber: number) => setCurrentPage(pageNumber); // Specify the type for pageNumber
